...
def start():
    ...
    client_conn ,client_addr = casino.accept()
    print("connected to client ")
    #recieve number of rounds from client
    rounds_data = client_conn.recv(1000)
    rounds = pickle.loads(rounds_data)
    #recieve number of players from client
    players_data = client_conn.recv(1000)
    num_players = pickle.loads(players_data)
    print("number of rounds set by client: ",rounds)
    print("number of players set by client: ",num_players)
    ...
    for i in range(num_players):
        ...
        player_connection_array[i].send(rounds_data)
    print("All connections established and round data sent")
